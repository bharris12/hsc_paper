...
if(!file.exists('rf1_raw.loom')){
  system('curl -O ftp://gillisdata.cshl.edu/data/HSC_atlas/rf1_raw.loom')
}
...
sce <-
  get_sce_alt('rf1_raw.loom')
ds_meta <- metadata %>% filter(study_id == 'rf_LARRY1' &
...
cds <- build_cds(sce, scale = F)

cds <- monocle_graph(cds, minimal_branch_len = 50)
...
path_vect <- get_eryth_path(cds, root_node  , 'Y_101')
...
ps_fn <-
  '/home/bharris/pseudotime/data/monocle/rf_LARRY1_erythroid_psuedotime.csv'
ps<-cbind(pseudotime(cds),path_vect)
compare_ps(cds,ps_fn)
saveRDS(cds,
        '~/pseudotime/data/monocle/rf_LARRY1_cds_reduced_complexity.RDS')

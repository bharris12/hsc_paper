...
if(!file.exists('weinreb_t9_raw.loom')){
  system('curl -O ftp://gillisdata.cshl.edu/data/HSC_atlas/weinreb_t9_raw.loom')
}
metadata <-
  read.csv(
...
sce <-
  get_sce_alt('weinreb_t9_raw.loom')


ds_meta <- metadata %>% filter(study_id == 'weinreb9' &
...
cds <- monocle_graph(cds, minimal_branch_len = 130)
...
#path_vect <- get_eryth_path(cds, root_node  , 'Y_627') 

#Alterative might be 707 or 627 (something to look at)
path_vect <- get_eryth_path(cds, root_node  , 'Y_4') 
...
ps_fn <-
  '/home/bharris/pseudotime/data/monocle/weinreb9_erythroid_psuedotime.csv'
# write.table(pseudotime(cds)[path_vect],
#             ps_fn,
#             sep = ',', col.names = FALSE)
ps<-cbind(pseudotime(cds),path_vect)
colnames(ps)<-c('pseudotime','eryth')
saveRDS(cds,
        '~/pseudotime/data/monocle/weinreb9_cds_reduced_complexity.RDS')

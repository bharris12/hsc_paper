...
if(!file.exists('tusi_batch1_raw.loom')){
  system('curl -O ftp://gillisdata.cshl.edu/data/HSC_atlas/tusi_batch1_raw.loom')
}
metadata <-
  read.csv(
...
sce <-
  get_sce_alt('tusi_batch1_raw.loom')
ds_meta <- metadata %>% filter(study_id == 'tusi_batch1' &
...
cds <- monocle_graph(cds, minimal_branch_len = 20)
...
path_vect <- get_eryth_path(cds, root_node  , 'Y_144')
...
ps_fn <-
  '/home/bharris/pseudotime/data/monocle/tusi_batch1_erythroid_psuedotime.csv'
ps<-cbind(pseudotime(cds),path_vect)
compare_ps(cds,ps_fn)
saveRDS(cds,
        '~/pseudotime/data/monocle/tusi_batch1_cds_reduced_complexity.RDS')
